# O valor do portfólio na era da AI

## 📌 Informações Gerais

- **Pessoa palestrante:** [Carlos Becker](../pessoas/carlos-becker.md)
- **Tipo de atividade:** Palestra / Talk
- **Categoria:** Carreira/Career
- **Nível:** Iniciante/Beginner
- **Experiência esperada em Open Source:** Nenhuma experiência/No experience needed
- **Idioma:** Português/Portuguese

## 📝 Descrição

Minha idéia é fazer uma provocação. Agora que "código é mato", qual o valor do OpenSource? O que diferencia um portfólio bom de um ruim?

Minha teoria é que é um misto de "quão boa essa idéia é", como o autor gerencia a comunidade em volta dos seus projetos (caso tenha uma), e talvez um pouco de "novelty".

A idéia tá bem fresca ainda, porém.

    ## 🎯 Principais aprendizados

    não sei ainda :D Acredito que algo no sentido de aproveitar a AI pra testar idéias, experimentar coisas, e resolver problemas reais.
